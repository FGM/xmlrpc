...
/**
 * @file
 * Contains \Drupal\xmlrpc_test\Tests\XmlRpcBasicTest.
 */
...
class XmlRpcBasicTest extends XmlRpcTestBase {
  ...
  public function testListMethods() {
    ...
    $methods = $this->xmlRpcGet(array('system.listMethods' => array()));
  ...
  public function testMethodSignature() {
    $signature = $this->xmlRpcGet(array('system.methodSignature' => array('system.listMethods')));
  ...
  public function testAcceptHeaders() {
    $request_header_sets = array(
      // Default.
      'implicit' => array(),
      'text/xml' => array('Accept' => 'text/xml'),
      'application/xml' => array('Accept' => 'application/xml'),
    );

    foreach ($request_header_sets as $accept => $headers) {
      try {
        $methods = $this->xmlRpcGet(array('system.listMethods' => array()), $headers);
  ...
  public function testInvalidServer() {
    ...
    $result = xmlrpc($invalid_endpoint, array('system.listMethods' => array()));
